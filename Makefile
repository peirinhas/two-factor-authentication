...
OS := $(shell uname)
DOCKER_BE = two-factor-authentication-api-be
...
ifeq ($(OS),Darwin)
	docker volume create --name=two-factor-authentication-api-be-sync
...
generate-ssh-keys: ## Generate ssh keys in the container
	...
	U_ID=${UID} docker exec -it --user ${UID} ${DOCKER_BE} openssl genrsa -passout pass:two-factor-authentication-api -out config/jwt/private.pem -aes256 4096
	U_ID=${UID} docker exec -it --user ${UID} ${DOCKER_BE} openssl rsa -pubout -passin pass:two-factor-authentication-api -in config/jwt/private.pem -out config/jwt/public.pem
